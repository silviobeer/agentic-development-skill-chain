...
cheap to decide, and every later step (1c, 1d, P0 preflight, the wave gate) reads
the outcome instead of re-asking:

- `docs/DESIGN-SYSTEM.md` present, missing, or over its 80-line cap
- `docs/components.md` generated (carries the `GENERATED by` marker), still
  hand-written from before the generator, or absent while components exist
...
If the scan found UI work without design rules — components in the code but no
`docs/DESIGN-SYSTEM.md`, or a greenfield UI project — put it to the user as a
decision here rather than assuming:
...
State` either way; later steps must not re-open a decision the user already made.
...
## Design System State
- `docs/DESIGN-SYSTEM.md`: present | missing | over cap (<N>/80 lines)
- `docs/components.md`: generated | hand-written (migration pending) | absent
