...
│     └── Stream 4: 6-Persona-Panel (Codex background OR Claude fallback)     │
...
│         • Ken Takahashi (Minimalism)  Code minimalism, YAGNI, duplication  │
│     │                                                                       │
│     ▼                                                                       │
│  [Critical/High Bugs?]                                                      │
│     │                                                                       │
...
flowchart TD
    ...
    subgraph QAStreams[4 parallel Streams + 6 Personas]
        ...
        Red[red-team-tester]
        UI[ui-auditor]
        Per[6-Persona-Panel<br/>Codex background<br/>oder Claude fallback]
    end
    S6 --> QAStreams
...
| **Was passiert** | |
| | **Phase 0 — Preflight (FIRST ACTION):** **Permissions:** Claude Code MUSS mit `claude --dangerously-skip-permissions` gestartet werden (echte Zero-Prompt-Garantie). Als Fallback/Belt-and-Braces: `bash scripts/merge-project-settings.sh` (merged Template mit `defaultMode: bypassPermissions` in `.claude/settings.json`, deny-list blockt destruktive Ops). **CodeRabbit-Config:** fehlt `.coderabbit.yaml` am Repo-Root → aus `~/.claude/skills/5_executing/references/coderabbit-template.yaml` kopieren (`chill` profile, path_filters, path_instructions). **MCP:** Supabase + Playwright verbunden (falls Features das brauchen). **Git-Tag Wave-1-Start:** `git tag wave-1-start-PROJ-<X>` am Feature-Start (Wave 2+ setzt `wave-gate.sh` automatisch). Dann `/compact`. |
...
| | &nbsp;&nbsp;4. Wave-Ende: Ralph ✓ → Build ✓ → CodeRabbit ✓ → Smoke Test ✓ → **Ken Minimalism Review** (via Codex-Companion-Script im Background oder Claude-Subagent) ✓ → **Pre-Gate: Post-Wave-Notes im Wave-Plan-File** → **`bash scripts/wave-gate.sh <N> <PROJ-X> <thema>`** (HARD-GATE, Exit 0) |
...
| | • `CLAUDE.md` bleibt pointer-only und verweist verpflichtend auf `AGENTS.md`; keine kuratierten Regeln in `CLAUDE.md` |
| **Output** | README.md / docs/PROJECT.md / docs/TECHNICAL.md / AGENTS.md / pointer-only CLAUDE.md — nur geänderte Files. Ein Commit: `docs(PROJ-<X>): Update project documentation` |
...
Orchestriert Skills 5 → 6 → 7 ohne User-Prompts. Setzt `CLAUDE_AUTONOMOUS_LEVEL=conservative|balanced|aggressive` (Default **balanced**) und ruft die drei Skills sequenziell auf. Die Skills prüfen die Env-Variable an allen `AskUserQuestion`-Callsites und ersetzen sie durch Default-Policy + Audit-Log-Eintrag.
...
| Decision | Policy |
|---|---|
| Ken Critical/High/Medium | auto-fix, halt nach 3 fehlgeschlagenen Fixes |
...
───────  ──────────────────────  ────────────  ──────────────────────────────────────
  ...
  6      qa                      Opus (Lead)   red-team-tester (Sonnet)
                                               ui-auditor (Sonnet)
                                               6 Persona Reviewer inkl. Ken (Codex oder Claude):
...
- **Wo:** `specs/`, `agent.md`, `progress.md`, `SKILL.md`, `.claude/agents/`, `.claude/skills/`
...
```
1. Ralph Loop              — AC-Verifikation mit echten Test-Commands
...
| Ken PROJ-End Review | `BUG-PROJ<X>-QA-KEN-<NNN>` | `BUG-PROJ1-QA-KEN-003` |
...
**Gilt für beide Gates:** Skill 5 (Ken-Findings pro Wave) und Skill 6 (QA-Persona-Bugs nach Quality Gate). In `conservative` Autonom-Level wird ebenfalls parallelisiert, aber bei erstem Cluster-Fail gestoppt statt weiterzulaufen.
...
├── CLAUDE.md                       (pointer-only; muss auf AGENTS.md verweisen)
...
└── src/
    ...
    └── features/<name>/
        └── agent.md                (Agent-Notizen, von Implementern + Ken gepflegt, Skill 7 harvestet in TECHNICAL.md)
```
...
| **`coderabbit`** | Empfohlen | Wave-Gate (Skill 5) — `coderabbit review --agent --base-commit $WAVE_BASE`. Fehlt → wave-gate.sh skipped diesen Check mit Warnung. | [coderabbit.ai/cli](https://coderabbit.ai) |
...
Fehlende MCP beim Preflight-Check → **STOP**, Skill 5 halted mit User-Hinweis "reconnect via `claude mcp list`/`claude mcp add`".
...
### Projekt-lokale Scripts (aus `~/.claude/skills/5_executing/scripts/`)
...
| File | Zweck | Template |
|---|---|---|
| **`.coderabbit.yaml`** | Per-Projekt CodeRabbit-Config (profile: chill, path_filters, path_instructions) | `~/.claude/skills/5_executing/references/coderabbit-template.yaml` |
| **`.claude/settings.json`** | Permissions (Allow/Deny/defaultMode) | `~/.claude/skills/5_executing/references/project-settings-template.json` |
| **`specs/PROJ-<X>-<thema>/6_plan/wave-gate-config.json`** | AC-Commands + frontend_routes pro Wave | Skill 4 generiert |
