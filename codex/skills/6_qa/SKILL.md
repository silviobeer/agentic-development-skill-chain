...
- Do not use Claude `Agent` tool syntax, `subagent_type`, `run_in_background`, `/compact`, or Claude permission modes.
...
- Collect candidates for project-root `AGENTS.md`.
...
Look for a design system reference (check `.codex/skills/references/design-system.md` in the project first, then `~/.codex/skills/references/design-system.md` globally). If found, audit the implemented UI for violations:
...
2. **Run clusters in parallel only when safe and allowed.** If Codex delegation is allowed, spawn one worker per disjoint file cluster. Otherwise hand the clusters back to implementation or fix locally only after the user explicitly asks for fixes.
