...
- No AskUserQuestion tool: ask the open stack layers as numbered question
  blocks in the conversation and wait for the answers before scaffolding.
- `CLAUDE.md` is only written when the project is also used from Claude
  Code. `AGENTS.md` is read natively and is the file that matters here.
- Everything else — the Stack table, the scaffold verification, the caps —
  is identical.
