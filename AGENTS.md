...
- Include only the core 0-to-8 chain (incl. 0b_intake and 3a_cross-review) and explicitly documented optional skills in this repository.
...
- Keep the framework helper scripts byte-identical across their skill copies (`state.sh`, `preflight.sh`, `ponytail-check.sh`, `compile-context-bundles.mjs`, `context-injector.mjs`, the role manifests, `curation-caps.sh` ×4, `intake-seal-check.sh`, the 3a_cross-review scripts/template, `agent-md-entry.md.tmpl`, `ledger.mjs`, `harvest-debt.sh`, the 8_delivery scripts/templates); `validate.sh` enforces this. `wave-gate.sh` is the exception — it diverges per platform.
