...
**How:** A thin runner starts a fresh Claude session per phase
(handoff only via `state.json` — no compact roulette). User stories run
in dependency waves; every wave is checked by a hard gate
(ACs/tests, build, CodeRabbit review, Sonar secrets scan, browser smoke).
Waves run sequentially on the PROJ branch by default; parallel
execution is an opt-in optimization (shared-checkout teams for
file-disjoint stories; isolated worktrees + merge gate only if
 ...
 (A)/Jira (TODO)  │                                                │
  ─► P2d ─► P3 ─► P4                                               │
     ...
     wave plans      │ branch  │  │  implementers │  └─┬──┘  └─┬──┘  └─┬──┘
                     ...
                     │ hooks   │  │ sequential by │  dedupe  curation probe,
                     │ preflt. │  │ default, par- │  → fix   (docs/,  PR body,
                     └─────────┘  │ allel opt-in  │  → debt  agent.md) CI checks
                                  └───────────────┘
   ─────────────────────────────────────────────────────────────────────────
   Cross-cutting: state.json (1 session per phase) · findings.json (1 ledger)
                  morning report + push at run end · stop = rescue + report
   Context:       docs/ (curated, injected) · specs/PROJ-X/ (short-lived)
...
4. **Context economy: as much as necessary, as little as possible.**
   Four mechanisms: (a) one phase = one fresh session, handoff only via
   `state.json` + file paths; (b) injection is tiered by spawn class —
...
```
  ...
  │     (loop over waves) │  sequential by default; parallel opt-in (§6)
  └───────────┬───────────┘
  ┌───────────▼───────────┐
...
Cross-cutting across all phases: `state.json`, `findings.json`,
...
the chain format at `specs/PROJ-X/3_PRDs/*.md` (user stories with
...
the files in `specs/PROJ-X/3_PRDs/` simply ARE the snapshot. **The new
...
2. **Normalize the snapshot:** convert into the chain PRD format
   (`specs/PROJ-X/3_PRDs/*.md`, user stories with Given/When/Then +
...
1. **Read the baseline:** `docs/ARCHITECTURE.md`. If it is missing →
   ...
   the code.
2. **Draft the delta:** `specs/PROJ-X/architecture-delta.md` — only the
...
- NEW: execution-mode decision per wave (§6): sequential (default) /
  shared-checkout parallel — only with a listed proof that the
...
3. **Write the decision log:** `specs/PROJ-X/decisions.md` — every
...
5. **Seal the approval:** `state.json` → `approved` with a decision-log
...
### P0 — Setup (new; replaces the FIRST-ACTION block from Skill 5)
...
3. **Generate the ground file** (common-ground principle): surface
   ...
   the codebase
4. **Activate the context-injector hook** (SubagentStart, type-scoped):
   injects the matching pack parts per agent type. The minimalism ladder
   does NOT come from here but from the ponytail plugin (§7)
5. **ponytail preflight:** plugin installed? Mode `full` active?
   `PONYTAIL_SUBAGENT_MATCHER` set to code-writing agent types?
6. Preflights: check the CLI/tool list from §7 (including auth, not just
   `command -v`), plus `.coderabbit.yaml` and permissions
...
```
for each wave N:
  1. Spawn implementers (execution mode per wave plan, §6)
     – Mode 1 sequential (DEFAULT) → directly on the PROJ branch
     – Mode 2 parallel, file-disjoint → agent team, shared checkout,
       integration-guard observes (today's model)
     – Mode 3 parallel, opt-in → own git worktree + story branch each
     – context arrives via hook; local knowledge via the agent.md
       reading protocol
  2. Per US: TDD + inner review loop (as today)
...
Three parallel collection streams:
...
Then: **ledger processing — strictly divided to protect the
...
- **Spawned verifier batches:** weed out false positives
  ...
  inline; the orchestrator only receives verdicts.
- **Fix spawns (tier 0)** per cluster; re-check via script.

Exit: no Critical/High open. Medium/Low → **auto-defer**
...
2. **Curation step (the brownfield core):**
   ...
   - `docs/GUIDELINES.md` + merge AGENTS.md candidates (pipeline exists)
...
### Session Architecture: One Phase = One Session
...
fresh sessions** — a thin runner starts a new session per phase
(P0, P5 possibly per wave, P6, P7, P8):
...
- Every session loads only what its phase needs: the P6 session knows
  no wave implementation history, the P8 session only ledger +
  state.json + diff.
...
|---|---|---|---|---|
| 1. Curated repo docs | `docs/` | forever | hook injection | only via P7 curation |
| 2. PROJ artifacts | `specs/PROJ-X/` | until merge | hook / spawn prompt | during the PROJ |
| 3. Local knowledge | `src/**/agent.md` | forever | reading protocol (explicit) | immediately on learning; curated in P7 |
...
├── components.md              ← component registry (exists)
└── AGENTS.md                  ← project-wide agent rules (pipeline exists)

specs/PROJ-X/                  ← SHORT-LIVED
├── 3_PRDs/                    ← as today
...
Implementation: **the tier is encoded in the agent type** — own
definitions in `.claude/agents/` (`micro-fixer` = tier 0,
`implementer`/`frontend-`/`backend-implementer` = tier 1,
`reviewer-*` = tier 2). The SubagentStart hook reliably receives only
the `agent_type` (ponytail pattern), not the prompt text — hence no
prompt marker. Side effect: agent definitions carry model + tool
restrictions along (tier 0 needs no WebSearch, for example).
...
| GUIDELINES.md | ✅ | ✅ |
| Minimalism ladder | via ponytail PLUGIN (own hooks, §7) — NOT from the injector, no double injection ||
...
**Sequential is the DEFAULT execution mode.** Worktree parallelism is
an opt-in optimization, built only when telemetry proves the need
(see execution modes below). Rationale: the normal case is the
overnight run, where wall-clock time is the cheapest resource —
parallelism does not reduce token cost or improve quality, but it adds
the concept's most fragile machinery (merge gate, contracts discipline,
per-worktree infra, a semantic merge conflict at 3 a.m.). Applying our
own minimalism ladder: it does not need to exist until proven otherwise.

```
main
└── proj/PROJ-X                     ← PROJ branch = the eventual PR
    ├── Mode 1 (DEFAULT): all waves sequential
    │                               → commit directly, tags as wave boundaries
    ├── Mode 2: parallel wave, file-disjoint stories
    │                               → agent team in the SHARED checkout
    │                                 (today's Skill 5 model, no worktrees)
    └── Mode 3 (OPT-IN, Stage 4): parallel wave with overlap risk
...
- **Mode 1 — sequential (DEFAULT):** all stories on the PROJ branch,
  one after another. No worktrees, no merge gate, no per-story infra;
  the agent.md broadcast works naturally. Depends on ZERO unverified
  assumptions. This is the mode the chain is tested and run in first.
- **Mode 2 — shared-checkout parallelism (cheap middle tier):** the
  ...
  disjoint (it must list them). Stories then run as an agent team in
  the shared checkout with the integration-guard as observer — exactly
  today's Skill 5 model. No branches, no merge gate; near-zero extra
  machinery.
- **Mode 3 — worktree parallelism (OPT-IN, evidence-gated):** built in
  Stage 4 only if telemetry from Stages 1–3 shows the nightly window
  actually overflows (PROJ not finished by morning). Everything below
  describes this mode.
...
- **Order per parallel wave:** per-US Ralph runs IN the worktree
  ...
  branch.
- The integration-guard turns from observer into the **merge gate**:
  merges in dependency order, fixes trivial conflicts, escalates
  semantic ones. Additionally: **union merge of the agent.md files**.
...
## 6b. Mapping to Claude Code

All building blocks of the git/subagent strategy rest on existing
primitives (`git`/`gh` CLI are available on the machine):

| Concept element | Claude Code primitive | Status |
|---|---|---|
| Subagents with type + model choice | `Agent` tool (`subagent_type`, `model`) | ✅ Skill 5 uses it today |
| Background spawns / batches | `run_in_background: true` | ✅ |
| Tier-/type-scoped injection | SubagentStart hook (`agent_type` in input) | ✅ proven by ponytail |
| Own agent types per tier | `.claude/agents/*.md` (model, tools) | ✅ |
| Wave-gate enforcement | PreToolUse hook | ✅ exists (`wave-gate-enforcer.js`) |
| Story worktrees, merge gate, rescue branch | plain git via Bash | ✅ |
| P8 conflict probe | `git worktree add --detach` + `merge --no-commit` | ✅ |
| PR + CI polling | `gh pr create` / `gh pr checks` | ✅ |
| Phase runner | bash script + headless `claude -p` per phase; kickoff via cron/schedule | ✅ |
| Push notification (morning report) | `PushNotification` tool | ✅ |
| Statusline from state.json | `statusLine` config | ✅ |
| Team messaging (gotcha broadcast) | agent teams | ✅ Skill 5 uses it today |

**Verify spike (~1h, 2 dummy stories) — required only if/when Mode 3
(worktree parallelism, Stage 4) is activated; Modes 1 and 2 depend on
zero unverified assumptions:**
1. Does SubagentStart also fire for team teammates? (proven only for
   Task spawns). Fallback: put the bundle into the teammate prompt.
   (Also relevant for Mode 2, which uses teams — verify then at latest.)
2. Teams × manual worktrees: direct a teammate into a foreign directory
   via prompt + messaging in parallel; while at it, play through the
   npm install strategy (pnpm shared store) and port allocation.
...
| `claude` | phase runner: headless `claude -p` per phase | ✅ hard |
...
| ponytail plugin | minimalism ladder via its own hooks (no CLI; needs `node`) | ✅ decided (§7 below) |
...
instead, ponytail is installed as a plugin and delivers it via its own
...
- **Mode:** persist `full` as the default (`/ponytail default full`).
- **Scoping:** set `PONYTAIL_SUBAGENT_MATCHER` to code-writing agent
  types (`implementer|frontend-implementer|backend-implementer|micro-fixer`)
  — reviewer, QA, and explore spawns do not get the ladder
  (tier logic stays consistent, no unnecessary context).
...
- **Update policy:** check plugin updates occasionally; on breaking
  changes in hook behavior the P0 preflight decides (pinning the
  version is an option if it ever becomes necessary).
...
| `compile-context-bundles.mjs` (4b_setup) | `docs/*`, `specs/PROJ-X/*`, injection matrix §5 | one bundle file per agent type under `.claude/context-bundles/` | counts tokens per bundle; exit ≠ 0 if any bundle exceeds budget (~2–3k) — condense first |
...
| `run-phase.sh` (framework root) | phase id, PROJ | launches headless `claude -p` with the phase prompt | reads state.json to validate the phase is legal next; records session start/end in state.json |
...
| `stop-report.md.tmpl` (framework) | state.json + verbatim error capture | `specs/PROJ-X/7_progress/stop-report.md` | on stop condition |
| `decisions.md.tmpl` (4a_checkpoint) | reconcile-loop results | `specs/PROJ-X/decisions.md` (append per checkpoint) | CP1/bootstrap/CP2 |
...
3. **Write the stop report** (`specs/PROJ-X/7_progress/stop-report.md`):
...
**Delivery:** file + push notification at run end (Claude Code
PushNotification) with a one-liner status ("2 PROJs done, 2 PRs open,
1 stop report") and the path to the report.
...
| Worktrees + merge gate for parallel waves | integration-guard as a mere observer |
...
1. **Stage 1 — make it overnight-capable:** autonomy policy (unify the
   escalation rules in Skill 5/6, stop report, morning report)
   + P8 delivery skill (incl. the CP2 reconcile loop for PR comments)
   + findings ledger (dedup/rendering as scripts)
   + minimal `state.json` + **phase runner** (session per phase,
...
4. **Stage 4 (OPT-IN, evidence-gated):** built only if telemetry from
   Stages 1–3 shows the nightly window overflowing. Verify spike (§6b)
   → worktree branching + merge gate (Mode 3), parallelism cap,
   DB/install/port strategy. Until then, Mode 2 (shared-checkout teams
   for file-disjoint waves) is the only parallelism.
...
## 11. Open Questions (under iteration)

*All concept questions are decided (as of v0.3):*
...
| Platform | GitHub + CI given; P8 polls via `gh pr checks` |
